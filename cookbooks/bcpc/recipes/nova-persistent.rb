...
ruby_block "setup-nova-persistent-config" do
  block do
...
template "/tmp/nova-persistent.sh" do
  user 'root'
  source "nova-persistent.sh.erb"
  owner "root"
  group "root"
  mode 0755
end

bash "nova-persistent-create" do
  user "root"
  code <<-EOH
    /tmp/nova-persistent.sh
  EOH
end
