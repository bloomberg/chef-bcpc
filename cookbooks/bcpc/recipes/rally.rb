...
bash 'create virtual env for rally' do
  code <<-EOH
    ...
    pip install --no-index --find-links=/tmp/rally --user virtualenv
...
bash 'install-rally' do
  code <<-EOH
    #{rally_venv_dir}/bin/pip install --no-index --find-links=/tmp/rally pbr cffi
    #{rally_venv_dir}/bin/pip install --no-index --find-links=/tmp/rally rally
  EOH
  user rally_user
end
